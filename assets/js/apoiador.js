/* =====================================================================
   ...
   - Cada formato usa uma moldura PNG (assets/img/molduras/*.png) com
     ...
     não existe, o script desenha uma moldura provisória com a identidade.
   - Para trocar a arte: substitua os PNGs e ajuste as coordenadas em
     FORMATOS (cx, cy, r do círculo da foto; caixa do nome).
   ===================================================================== */
(function () {
  "use strict";
  ...
  var FORMATOS = [
    { key: "feed", label: "Feed do Instagram e Facebook", dim: "1080 × 1350", w: 1080, h: 1350,
    ...
    { key: "story", label: "Story do Instagram, Facebook e WhatsApp", dim: "1080 × 1920", w: 1080, h: 1920,
    ...
    { key: "perfil", label: "Foto de perfil do WhatsApp, Instagram e Facebook", dim: "1080 × 1080", w: 1080, h: 1080,
  ...
  mount.innerHTML =
    ...
    '    </label>' +
    '    <div class="ap__step ap__hidden" id="ap-step2"><span class="ap__num">2</span><div><strong>Ajuste o enquadramento</strong><p>Arraste a foto para posicionar e use o zoom.</p></div></div>' +
    ...
    '      <div class="ap__zoom"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg><input id="ap-zoom" type="range" min="1" max="3" step="0.01" value="1" aria-label="Zoom da foto"><button type="button" class="ap__link" id="ap-trocar">Trocar foto</button></div>' +
    '    </div>' +
    '    <div class="ap__step ap__hidden" id="ap-step3"><span class="ap__num">3</span><div><strong>Seu nome (opcional)</strong><p>Aparece na arte do feed e do story.</p></div></div>' +
  ...
  var fileEl = document.getElementById("ap-file"), drop = document.getElementById("ap-drop");
  var prev = document.getElementById("ap-prev"), pctx = prev.getContext("2d");
  var frameEl = document.getElementById("ap-frame"), zoomEl = document.getElementById("ap-zoom");
  var nomeEl = document.getElementById("ap-nome"), gerarEl = document.getElementById("ap-gerar");
  var results = document.getElementById("ap-results"), vazio = document.getElementById("ap-vazio");
  var show = function (id) { document.getElementById(id).classList.remove("ap__hidden"); };

  var foto = null, st = { z: 1, ox: 0, oy: 0 };
  ...
  FORMATOS.forEach(function (f) {
    ...
    im.onload = function () { molduras[f.key] = im; };
  ...
  function carregar(file) {
    ...
    var done = function (img) {
      foto = img; st = { z: 1, ox: 0, oy: 0 }; zoomEl.value = 1;
      show("ap-step2"); show("ap-ajuste"); show("ap-step3"); show("ap-nome"); show("ap-gerar");
      drop.classList.add("ap__drop--ok");
      drop.querySelector(".ap__drop-txt").innerHTML = "<b>Foto enviada!</b><br>Clique para escolher outra";
      desenharPreview();
      document.getElementById("ap-step2").scrollIntoView({ behavior: "smooth", block: "center" });
    };
  ...
  document.getElementById("ap-trocar").addEventListener("click", function () { fileEl.value = ""; fileEl.click(); });
  ...
  /* ---------- preview circular + arrastar/zoom ---------- */
  ...
  function desenharPreview() {
    ...
    pctx.fillStyle = "#EFE6FA"; pctx.fillRect(0, 0, prev.width, prev.height);
    if (foto) fotoCirculo(pctx, prev.width / 2, prev.height / 2, prev.width / 2, foto, st);
  }
  zoomEl.addEventListener("input", function () { st.z = parseFloat(zoomEl.value); desenharPreview(); });
  (function drag() {
    var on = false, lx = 0, ly = 0, span = 140;
    frameEl.addEventListener("pointerdown", function (e) { if (!foto) return; on = true; lx = e.clientX; ly = e.clientY; frameEl.setPointerCapture(e.pointerId); frameEl.style.cursor = "grabbing"; });
    frameEl.addEventListener("pointermove", function (e) {
      ...
      lx = e.clientX; ly = e.clientY; desenharPreview();
    });
    var end = function () { on = false; frameEl.style.cursor = "grab"; };
    frameEl.addEventListener("pointerup", end); frameEl.addEventListener("pointercancel", end);
    frameEl.addEventListener("wheel", function (e) { if (!foto) return; e.preventDefault(); st.z = Math.max(1, Math.min(3, st.z - e.deltaY * 0.002)); zoomEl.value = st.z; desenharPreview(); }, { passive: false });
  })();
  ...
  function frenteProvisoria(ctx, f) {
    ...
    if (f.key === "perfil") {
      // faixa inferior com o número
      var bw = 660 * s, bh = 170 * s, bx = (f.w - bw) / 2, by = f.h - bh - 42 * s;
      roundRect(ctx, bx, by, bw, bh, bh / 2); ctx.fillStyle = CORES.roxo; ctx.fill();
      ctx.lineWidth = 8 * s; ctx.strokeStyle = CORES.branco; ctx.stroke();
      ctx.fillStyle = CORES.branco; ctx.font = "900 " + 30 * s + "px Montserrat, Arial, sans-serif"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.letterSpacing = "4px";
      ctx.fillText("EU APOIO MÍRIAM RIBAS", f.w / 2, by + 48 * s);
      ctx.fillStyle = CORES.rosa; ctx.font = "" + 92 * s + "px 'Titan One', Montserrat, Arial, sans-serif"; ctx.letterSpacing = "2px";
      ctx.fillText(CAMPANHA.numero, f.w / 2, by + 118 * s);
      // anel externo grosso
      ctx.save(); ctx.strokeStyle = CORES.roxo; ctx.lineWidth = 46 * s; ctx.beginPath(); ctx.arc(f.foto.cx, f.foto.cy, f.foto.r - 23 * s, 0, Math.PI * 2); ctx.stroke();
      ctx.strokeStyle = CORES.rosa; ctx.lineWidth = 14 * s; ctx.beginPath(); ctx.arc(f.foto.cx, f.foto.cy, f.foto.r - 53 * s, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
      return;
    }
    // "EU APOIO" + tagline (posições em f.prov, pensadas para cada formato)
    var p = f.prov;
    ...
    Promise.all([fontes].concat(pendentes)).then(function () {
      ...
      FORMATOS.forEach(function (f) {
        ...
        canvas.toBlob(function (blob) {
          ...
          card.appendChild(acts); results.appendChild(card);
